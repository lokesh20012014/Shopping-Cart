...
{
$loginsql = "SELECT * FROM `shopping`.`logins` WHERE username = '" . $_POST['userBox']. "' AND password = '" . sha1($_POST['passBox']) . "'";
$loginres = mysqli_query($success, $loginsql);
...
$ordersql = "SELECT id FROM `shopping`.`orders` WHERE customer_id = " . $_SESSION['SESS_USERID'] . " AND status < 2";
$orderres = mysqli_query($success, $ordersql);
